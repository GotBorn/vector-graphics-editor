...
static void cls_OnKey( HWND hWnd, UINT vk, BOOL down, int repeat, UINT flags )
{
    ...
    case VK_F10:
        pWndData->kbd_queue.push( (TCHAR)0 );
        pWndData->kbd_queue.push( (TCHAR)KEY_F10 );
        SetEvent( pWndData->key_waiting );
        break;
    case VK_F11:
        pWndData->kbd_queue.push( (TCHAR)0 );
        pWndData->kbd_queue.push( (TCHAR)KEY_F11 );
        SetEvent( pWndData->key_waiting );
        break;
    case VK_F12:
        pWndData->kbd_queue.push( (TCHAR)0 );
        pWndData->kbd_queue.push( (TCHAR)KEY_F12 );
        SetEvent( pWndData->key_waiting );
        break;
    case VK_NUMLOCK:
        pWndData->kbd_queue.push( (TCHAR)0 );
        pWndData->kbd_queue.push( (TCHAR)KEY_NUM_LOCK );
        SetEvent( pWndData->key_waiting );
        break;
    case VK_SCROLL:
        pWndData->kbd_queue.push( (TCHAR)0 );
        pWndData->kbd_queue.push( (TCHAR)KEY_SCROLL_LOCK );
        SetEvent( pWndData->key_waiting );
        break;
    case VK_SHIFT:
        pWndData->kbd_queue.push( (TCHAR)0 );
        pWndData->kbd_queue.push( (TCHAR)KEY_SHIFT);
	pWndData->flags|=16;
        SetEvent( pWndData->key_waiting );
        break;
    case VK_CONTROL:
        pWndData->kbd_queue.push( (TCHAR)0 );
        pWndData->kbd_queue.push( (TCHAR)KEY_CTRL);
	if(flags&0x100) pWndData->flags|=4; else pWndData->flags|=8;
        SetEvent( pWndData->key_waiting );
        break;
    case VK_ALT:
        pWndData->kbd_queue.push( (TCHAR)0 );
        pWndData->kbd_queue.push( (TCHAR)KEY_ALT);
	if(flags&0x100) pWndData->flags|=1; else pWndData->flags|=2;
        SetEvent( pWndData->key_waiting );
        return;
    }
...
( HWND hWnd, UINT uiMessage, WPARAM wParam, LPARAM lParam )
{
	...
	{
	    if(lParam&0x1000000) pWndData->flags&=~4; else pWndData->flags&=~8;
	}
	...
	{
	    if(lParam&0x1000000) pWndData->flags&=~1; else pWndData->flags&=~2;
            return 0;
	}
	break;
